...
{
    public class TemplateSource : Source
    {
        ...
        {
            await Task.Yield();
            WmlDocument = DocumentAssembler.AssembleDocument(TemplateDoc, Data, out DocumentAssembler.AssembleResults results);
    ...
    public static class DocumentComposer
    {
        ...
        public static async Task<WmlDocument> ComposeDocument(WmlDocument templateDoc, XmlDocument data, List<Source> insertSources)
        {
        ...
        public static async Task<WmlDocument> ComposeDocument(WmlDocument templateDoc, XElement data, List<Source> insertSources)
        {
            await Task.Yield();
            var sourcesProvided = insertSources != null && insertSources.Count > 0;
            // assemble document
            var mainDoc = DocumentAssembler.AssembleDocument(templateDoc, data, out DocumentAssembler.AssembleResults results);
        ...
        public static async Task<WmlDocument> ComposeDocument(List<Source> sources)
        {
            var errors = new List<string>();
        ...
        private static Source GetSourceFromInsertResult(DocumentAssembler.AssembleInsert insert, string dirName)
        {
            ...
            if (filename != null && File.Exists(filename))
            {
                if (insert.Data == null)
                {
                    return new Source(filename, insert.Id);
                }
                else
                {
